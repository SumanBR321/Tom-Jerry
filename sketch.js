...
function preload(){
  jerryImg = loadAnimation("../images/jerryStanding.png");
...
function setup(){
    ...
    invisibleGround = createSprite(width/2,height-60,2000,10);
    ...
    if (gameState===PLAY){
     ...
     // fontsize(8);
      ...
      if(keyDown("space") && jerry.y >= height-180) {
        jerry.velocityY = -44;
        jerry.changeAnimation("jump",jerry);
      }
      else{ 
        jerry.changeAnimation("jerry",jerry);
      }
      jerry.velocityY = jerry.velocityY + 3;
...
function spawnObstacles() {
  if(frameCount % 125 === 0) {
    var obstacle = createSprite(2000,height-100,10,40);
    obstacle.velocityX = -(14 + 3*score/100);
